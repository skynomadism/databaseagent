...
namespace DataBaseAgent
{
    public class MySqlDBOperator : DBOperator
    {
        ...
        private MySqlTransaction trans; //事务处理类 
        ...
        {
            conn = new Conn().GetMySqlConn();
        }
        public MySqlDBOperator(int dbID)
        {
            conn = new Conn().GetMySqlConn(dbID);
        }
        public MySqlDBOperator(string ConnectString) 
        {
            conn = new Conn().GetMySqlConn(ConnectString);
        }
        ...
        /// <returns></returns>
        private MySqlCommand CreateCommand(string SQLstring, CommandType CommandType, string[,] SqlParams)
        {
            ...
            if (SqlParams != null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    {
                        cmd.Parameters.Add(SqlParams[i, 0], myDBConvert.ConvertMySqlDB(SqlParams[i, 1]), int.Parse(SqlParams[i, 2]), SqlParams[i, 3]);
                    }
        ...
        /// <param name="values">参数的值</param>
        /// <returns></returns>
        private MySqlCommand CreateCommand(string SQLstring, CommandType CommandType, string[,] SqlParams, object[] values)
        {
            myDBConvert = new TypeConvert();
            MySqlCommand cmd = new MySqlCommand();
            cmd.Connection = this.conn;
            ...
            if (SqlParams != null && values != null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    {
                        cmd.Parameters.Add(SqlParams[i, 0], myDBConvert.ConvertMySqlDB(SqlParams[i, 1]), int.Parse(SqlParams[i, 2]),
