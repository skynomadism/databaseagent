...
namespace DataBaseAgent
{
    class SqlDBOperator : DBOperator
    {
        ...
        {
            conn = new Conn().GetSqlConn();
        }
        public SqlDBOperator(int dbID)
        {
            conn = new Conn().GetSqlConn(dbID);
        }
        public SqlDBOperator(string ConnectString) 
        {
            conn = new Conn().GetSqlConn(ConnectString);
        }
        ...
        /// <returns></returns>
        private SqlCommand CreateCommand(string SQLstring, CommandType CommandType, string[,] SqlParams)
        {
            ...
            if (SqlParams != null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    {
                        cmd.Parameters.Add(SqlParams[i, 0], myDBConvert.ConvertSqlDB(SqlParams[i, 1]), int.Parse(SqlParams[i, 2]), SqlParams[i, 3]);
                    }
        ...
        /// <param name="values">参数的值</param>
        /// <returns></returns>
        private SqlCommand CreateCommand(string SQLstring, CommandType CommandType, string[,] SqlParams, object[] values)
        {
            myDBConvert = new TypeConvert();
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = this.conn;
            ...
            if (SqlParams != null && values != null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    {
                        cmd.Parameters.Add(SqlParams[i, 0], myDBConvert.ConvertSqlDB(SqlParams[i, 1]), int.Parse(SqlParams[i, 2]),
